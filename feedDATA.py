...
class ModelFeeder:
    ...
    def __init__(self):
        ...
        self.mouthtrain = r'C:\Users\gerar\PycharmProjects\TRAINMOUTH'
        self.mouthtest = r'C:\Users\gerar\PycharmProjects\TESTMOUTH'
        self.dirhold = r'C:\Users\gerar\PycharmProjects\PFOTOS'
        self.mouthhold = r'C:\Users\gerar\PycharmProjects\MFOTOS'
    ...
    def mouthPHOTO(self):
        ...
        while True:
            ...
            self.pfxname = 'm_00_'
            idx = 0
            trainlist = os.listdir(self.mouthtrain)
            testlist = os.listdir(self.mouthtest)
            ...
            if len(nodes) != 0:
                # Landmarks
                name = self.pfxname + str(int(idx + 1))
                savedat = os.path.join(self.mouthhold, name)
                np.save(savedat, nodes)
                # Images
                name = name + '.jpg'
                savedir = os.path.join(self.mouthhold, name)
                cv2.imwrite(savedir, frame)
            elif len(nodes) == 0:
                pass

            holditems = len(os.listdir(self.mouthhold))
            if holditems > 1049:
                self.sf.play()
                cap.release()
                # cv2.destroyAllWindows()
        ...
        while True:
            ...
            holditems = len(os.listdir(self.mouthhold)) - 1050
            if holditems > 1049:
                self.sf.play()
                cap.release()
                # cv2.destroyAllWindows()
        ...
        os.startfile(filepath=self.mouthhold)
    # ------------------------------------------------
    ...
    def loadMouthD(self):
        ...
        filelist = os.listdir(self.mouthhold)
        for file in filelist:
            if file.startswith('m_00') and file.endswith('.npy'):
                i0 += 1
            elif file.startswith('m_01') and file.endswith('.npy'):
                i1 += 1
            else:
                pass
        ...
        holdlist = os.listdir(self.mouthhold)
        for file in holdlist:
            if file.startswith('m_00') and file.endswith('.npy') and ip0 < i0:
                oldpath = os.path.join(self.mouthhold, file)
                newpath = os.path.join(self.mouthtrain, file)
                os.replace(oldpath, newpath)
                ip0 += 1

            if file.startswith('m_01') and file.endswith('.npy') and ip1 < i1:
                oldpath = os.path.join(self.mouthhold, file)
                newpath = os.path.join(self.mouthtrain, file)
                os.replace(oldpath, newpath)
                ip1 += 1
        ...
        for file in holdlist:
            if file.endswith('.npy'):
                oldpath = os.path.join(self.mouthhold, file)
                newpath = os.path.join(self.mouthtest, file)
                os.replace(oldpath, newpath)

        # Creación de labels TRAIN
        lbl = []
        filelist = os.listdir(self.mouthtrain)
        for file in filelist:
            if file.startswith('m_00'):
                lbl.append(0)
            elif file.startswith('m_01'):
                lbl.append(1)
            else:
                pass
        ...
        df = pd.DataFrame(list(zip(lbl)))
        with pd.ExcelWriter(self.dirxlsx, mode='a', if_sheet_exists='overlay') as writer:
            df.to_excel(writer, sheet_name='traintags', header=False, index=False, startcol=self.nmodel+1, startrow=1)

        # Creación de labels TEST
        lbl = []
        filelist = os.listdir(self.mouthtest)
        for file in filelist:
            if file.startswith('m_00'):
                lbl.append(0)
            elif file.startswith('m_01'):
        ...
        df = pd.DataFrame(list(zip(lbl)))
        with pd.ExcelWriter(self.dirxlsx, mode='a', if_sheet_exists='overlay') as writer:
            df.to_excel(writer, sheet_name='testtags', header=False, index=False, startcol=self.nmodel+1, startrow=1)
    ...
    def resetMaster(self):
        ...
        for file in filelist:
            if file.startswith('m_'):
                path = os.path.join(self.mouthtrain, file)
                os.remove(path)
                idx += 1
        ...
        for file in filelist:
            if file.startswith('m_'):
                path = os.path.join(self.mouthtest, file)
                os.remove(path)
                idx += 1
